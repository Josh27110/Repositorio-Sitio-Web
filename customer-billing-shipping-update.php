...
function isValidPhone($phone) {
    return preg_match("/^[0-9]+$/", $phone) && $phone > 0;
}

function isValidZip($zip) {
    return preg_match("/^[0-9]+$/", $zip) && $zip > 0;
}
...
if (isset($_POST['form1'])) {
    ...
    // Validaciones para los campos de facturación
    if (empty($_POST['cust_b_name']) || !isValidName($_POST['cust_b_name'])) {
        $valid = 0;
        $error_message .= "Nombre no válido.<br>";
    }

    if (!empty($_POST['cust_b_cname']) && !isValidName($_POST['cust_b_cname'])) {
        $valid = 0;
        $error_message .= "Nombre de compañía no válido.<br>";
    }

    if (empty($_POST['cust_b_phone']) || !isValidPhone($_POST['cust_b_phone'])) {
        $valid = 0;
        $error_message .= "Número de teléfono no válido.<br>";
    }

    if (empty($_POST['cust_b_address'])) {
        $valid = 0;
        $error_message .= "Dirección no válida.<br>";
    }

    if (empty($_POST['cust_b_country'])) {
        $valid = 0;
        $error_message .= "País no válido.<br>";
    }

    if (empty($_POST['cust_b_city']) || !isValidName($_POST['cust_b_city'])) {
        $valid = 0;
        $error_message .= "Ciudad no válida.<br>";
    }

    if (empty($_POST['cust_b_state']) || !isValidName($_POST['cust_b_state'])) {
        $valid = 0;
        $error_message .= "Estado no válido.<br>";
    }

    if (empty($_POST['cust_b_zip']) || !isValidZip($_POST['cust_b_zip'])) {
        $valid = 0;
        $error_message .= "Código postal no válido.<br>";
    }

    // Validaciones para los campos de envío
    if (empty($_POST['cust_s_name']) || !isValidName($_POST['cust_s_name'])) {
        $valid = 0;
        $error_message .= "Nombre no válido.<br>";
    }

    if (!empty($_POST['cust_s_cname']) && !isValidName($_POST['cust_s_cname'])) {
        $valid = 0;
        $error_message .= "Nombre de compañía no válido.<br>";
    }

    if (empty($_POST['cust_s_phone']) || !isValidPhone($_POST['cust_s_phone'])) {
        $valid = 0;
        $error_message .= "Número de teléfono no válido.<br>";
    }
    ...
    if (empty($_POST['cust_s_city']) || !isValidName($_POST['cust_s_city'])) {
        $valid = 0;
        $error_message .= "Ciudad no válida.<br>";
    }

    if (empty($_POST['cust_s_state']) || !isValidName($_POST['cust_s_state'])) {
        $valid = 0;
        $error_message .= "Estado no válido.<br>";
    }

    if (empty($_POST['cust_s_zip']) || !isValidZip($_POST['cust_s_zip'])) {
        $valid = 0;
        $error_message .= "Código postal no válido.<br>";
    }
